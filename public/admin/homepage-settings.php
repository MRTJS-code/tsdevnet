...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($fieldTypes as $key => $type) {
        if ($type === 'bool') {
            $submitted[$key] = !empty($_POST[$key]) ? '1' : '0';
            continue;
        }
    ...
    if (!$errors) {
        ...
        $audit->log('admin', $admin ? (int) $admin['id'] : null, 'homepage_settings_updated', ['keys' => array_keys($payload)], Util::clientIp());
        $settings = $submitted;
    } else {
        $settings = $submitted;
    }
...
View::render('admin/homepage_settings', [
    'title' => 'Homepage settings | ' . $app['config']['app_name'],
    'bodyClass' => 'page',
    'settings' => $settings,
    'errors' => $errors,
]);
